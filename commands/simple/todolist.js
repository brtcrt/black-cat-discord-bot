...
let todolist = require(path.join(__dirname, "../../") +
...
module.exports = {
    ...
    category: "Simple",
    usage: "todo (optional: add, remove, change).\nadd: todo add (todo)\nremove: todo remove (place in list eg. 1)\nchange: todo change (place in list) (new todo)",
    cooldown: 1,
    aliases: ["todolist"],
        ...
        if (todolist[message.author.id] && !message.author.bot && !args[0]) {
            if (todolist[message.author.id].length <= 0) {
                let nothinginlist = new Discord.MessageEmbed()
                    ...
                    .setColor("#f01717")
                    .setDescription(
                        "Looks like you don't have anything in your todo list. If you need help, use bot?"
                    );
            ...
            if (!addTodo) {
                let failMessage = new Discord.MessageEmbed()
                    ...
                    .setColor("#f01717")
                    .setDescription(
                        "You can't add an empty string to your todo list!"
                    );
            ...
            if (!args[1] || !Number.isInteger(parseInt(args[1]))) {
                let failMessage = new Discord.MessageEmbed()
                    ...
                    .setColor("#f01717")
                    .setDescription(
                        "You didn't specify which one to take off your todo list!"
                    );
                return message.channel.send(failMessage);
            }
            ...
            let removedMessage = new Discord.MessageEmbed()
                ...
                .setColor("#22ff00")
                .setDescription(
                    removedone + " was removed from your todo list."
                );
            ...
            message.channel.send({ embeds: [changedMessage] });
            fs.writeFile(
                path.join(__dirname, "../") + "/database/todolist.json",
                JSON.stringify(todolist, null, 4),
                (err) => {
                    if (err) console.log(err);
                }
