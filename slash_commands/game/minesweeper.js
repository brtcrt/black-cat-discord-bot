...
class minesweeper {
    ...
    move(row, col, interaction) {
        ...
        if (chosen_one.isBomb) {
            ...
            return interaction.channel.send({ embeds: [death_embed] });
        }
...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const update_board = async () => {
            await interaction.editReply({ embeds: [game.visualize()] });
        };
        ...
        messageCollector.on("collect", (msg) => {
            ...
            if (move_args.length === 2) {
                msg.delete();
                game.move(move_args[1], move_args[0], interaction);
            } else if (
                move_args.length === 3 &&
